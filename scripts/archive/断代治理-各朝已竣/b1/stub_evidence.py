# -*- coding: utf-8 -*-
"""B1 第三檔：找「整理本另立之無撰人 stub」與其同組同卷之有撰人條配對，
並以該 stub 所繫整理本 section 之正文是否書有候選之撰人名為證。只讀不寫。"""
...
# 建 work_id → [(整理本檔, 節題, 正文)] 之表，全庫只掃一次
...
for G in GS:
    ...
    for s in R:
        ...
        for p, t, body in sec.get(s['id'], []):
            for n in names:
                if n and n in body: hits.append((p.split('/')[-1], t, n, body[:120]))
        out.append({'title': G['title'], 'stub': s['id'], 'juan': s['juan'],
                    'stub_src': [e['source'] for e in s['indexed_by']],
                    'keeper': c['id'], 'names': names,
                    'keeper_src': [e['source'] for e in c['indexed_by']],
                    'evidence': hits, 'n_sec': len(sec.get(s['id'], []))})
