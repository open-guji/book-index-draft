...
用法：python3 scripts/apply_medium_review.py <results_dir> [--apply]
"""
...
def main():
    ...
    src = {}
    for f in sorted(glob.glob(os.path.join(rdir, 'mrev_*.json'))):
        if f.endswith('.result.json'):
            continue
        for e in json.load(open(f)):
            src[e['id']] = e
    dec = {}
    for f in sorted(glob.glob(os.path.join(rdir, 'mrev_*.result.json'))):
    ...
    with open('.claude/known-issues/A1-medium複審棄置.json', 'w', encoding='utf-8') as f:
