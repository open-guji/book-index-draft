...
2026-05-01 完成。問題是 `補晉書藝文志` 13 條 Work 的書名、官職、撰人、卷數發生串位；已刪除 3 條純注文，修復 10 條人名/書名，其中 5 條仍保留部分不確定撰人。詳見既有修復報告與歷史提交，不在 `todo.md` 展開受影響 wid 表。

---

### 殘名撰人 169 條（✅ 已完成）

已分批處理甲類、乙類、二字殘名與全庫普查，合計修復 200+ Work/Entity 關係，並清理一批錯誤 Entity 關聯。仍保留少量單字名或不可考殘名，不再作為主線待辦；如需回查細節，見 `.claude/known-issues/殘名撰人.md`、`.claude/known-issues/殘名撰人.json` 及各批次歷史提交。

---

### 秦漢同名異書核對（✅ 已完成主要批次）

已完成 16 批同名異書、連書省撰人、Entity 分裂/合併與全庫掃描，合併或修正數百條 Work/Entity 關係。剩餘多為大型異撰人集合或需外部考證的個案，暫不在 `todo.md` 展開；回查入口為 `.claude/known-issues/秦漢同名異書核對.md` 及各 batch JSON。
