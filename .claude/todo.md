...
更新：2026-08-09（宋代整理 Round 5：高置信人工詞典續查，26 Author/Work 拆分，11 Entity 拆分）
...
### 宋代整理 Round 2（✅ 本地完成，待推 PR）

**分支**：`fix/song-dynasty-round1`

**處理範圍**：繼續判定 Round 1 後殘留的 `Author.dynasty=宋`，只採用兩類高置信資料：
- 庫內同名已規範傳播：同名作者在 Entity 或其他 Work authors[] 中唯一對應 `北宋` / `南宋`
- 題名年號：題名含宋代年號且只命中北宋或南宋一側

**安全邊界**：
- 兩類證據衝突者跳過
- 已有 period 且非 `song` 的 Work 不處理
- 含 `[ ]`、`，`、`、`、`等` 的複合作者串不整條更新，避免把後代整理者一併標為宋

**腳本**：
- `scripts/fix_song_round2.py`

**已完成**：
| 類型 | 數量 | 說明 |
|---|---:|---|
| Author.dynasty 拆分 | 76 | `宋→北宋` 44、`宋→南宋` 32 |
| Work.dynasty 補齊 | 69 | `北宋` 38、`南宋` 31 |
| Work.period 補齊 | 68 | `北宋` 37、`南宋` 31 |
| Entity.dynasty 拆分 | 1 | `宋→北宋` 1 |
| index 同步 | 17 shards | `index/works` 16 shards，`index/entities` 1 shard |

**驗證**：
- `period=song` Work：3,050
- 宋代相關 Entity：987
- Work / Entity 與 index 分片不一致：0

**剩餘未決**：
- `Work.dynasty=宋`：1
- `Author.dynasty=宋`：1,097
- `Entity.dynasty=宋`：535
- 詳見 `.claude/known-issues/宋代整理_round2_未決.json`

---

### 宋代整理 Round 3（✅ 本地完成，待推 PR）

**分支**：`fix/song-dynasty-round1`

**處理範圍**：在 Round 2 後殘留項中，搜集外部資料建立小批人工詞典，只更新資料明確者：
- 人名/別稱 + 代表作可對應北宋/南宋者，如張子=張載、伊川程子=程頤、邵子=邵雍、陸遊、楊士瀛、黃昇、金履祥等
- 醫學、詞學、金石、筆記類高頻作者，如王惟一、陳自明、陳思、洪適、釋文瑩、劉道醇、董逌等
- 誤混入趙宋整理的南朝宋人物謝瞻
- `古三墳` 依描述中「北宋元豐七年（1084）」線索拆為北宋

**安全邊界**：
- 只處理 `scripts/fix_song_round3.py` 的 `MANUAL_FIGURES` 明列姓名
- `佚名`、`不著撰人` 等通用名不按姓名傳播；僅單獨處理 `古三墳`
- 同一 Work 的非通用作者若證據不唯一，不補頂層 dynasty

**腳本**：
- `scripts/fix_song_round3.py`

**已完成**：
| 類型 | 數量 | 說明 |
|---|---:|---|
| Author.dynasty 拆分 | 99 | `宋→北宋` 29、`宋→南宋` 65、`宋→南朝宋` 4，另 `古三墳` 佚名作者 1 條補北宋 |
| Work.dynasty 補齊 | 99 | `北宋` 30（含 `古三墳`）、`南宋` 65、`南朝宋` 4 |
| Work.period 補齊 | 99 | 北/南宋 → `song`，南朝宋 → `nanbeichao` |
| Entity.dynasty 拆分 | 22 | `北宋` 8、`南宋` 13、`南朝宋` 1 |
| index 同步 | 29 shards | `index/works` 16 shards，`index/entities` 13 shards |

**驗證**：
- 本輪觸及 Work：99，與 `index/works` 不一致：0
- 本輪觸及 Entity：22，與 `index/entities` 不一致：0
- `Work.dynasty=宋`：0

**剩餘未決**：
- `Author.dynasty=宋`：998
- `Entity.dynasty=宋`：513
- 詳見 `.claude/known-issues/宋代整理_round3_未決.json`

---

### 宋代整理 Round 4（✅ 本地完成，待推 PR）

**分支**：`fix/song-dynasty-round1`

**處理範圍**：在 Round 3 後殘留項中，繼續查核高頻作者及代表作，只收入有明確年代或南北宋線索者：
- 北宋：陳景元（熙寧五年 1072 進《道德真經藏室纂微篇》）
- 南宋：李曾伯、嚴羽、楊輝、鄭思肖、陳湻/陳淳、陳景沂、陳顯微、魏仲舉、宋伯仁、楊伯喦、葉士龍、王子俊、稅與權

**安全邊界**：
- 只處理 `scripts/fix_song_round4.py` 的 `MANUAL_FIGURES` 明列姓名
- 僅見「宋」而缺具體南北宋線索者（如劉球、江贄、林堯叟等）不強判
- 釋延壽等跨五代吳越/宋初、政治歸屬不穩者暫不歸北宋

**腳本**：
- `scripts/fix_song_round4.py`

**已完成**：
| 類型 | 數量 | 說明 |
|---|---:|---|
| Author.dynasty 拆分 | 43 | `宋→北宋` 3、`宋→南宋` 40 |
| Work.dynasty 補齊 | 43 | `北宋` 3、`南宋` 40 |
| Work.period 補齊 | 43 | 北/南宋 → `song` |
| Entity.dynasty 拆分 | 11 | `宋→南宋` 11 |
| index 同步 | 22 shards | `index/works` 15 shards，`index/entities` 7 shards |

**驗證**：
- 本輪觸及 Work：43，與 `index/works` 不一致：0
- 本輪觸及 Entity：11，與 `index/entities` 不一致：0
- `Work.dynasty=宋`：0

**剩餘未決**：
- `Author.dynasty=宋`：955
- `Entity.dynasty=宋`：502
- 詳見 `.claude/known-issues/宋代整理_round4_未決.json`

---

### 宋代整理 Round 5（✅ 本地完成，待推 PR）

**分支**：`fix/song-dynasty-round1`

**處理範圍**：在 Round 4 後殘留項中，繼續查核高頻且代表作明確的作者：
- 北宋：高承、魏泰、張伯端
- 南宋：王宗傳、李過、毛晃、葉紹翁、黃幹、魏峴、桂萬榮、陳鵠、費袞、康與之

**安全邊界**：
- 只處理 `scripts/fix_song_round5.py` 的 `MANUAL_FIGURES` 明列姓名
- 釋延壽、莊季裕等跨五代/靖康建炎或主活動期不穩者暫不處理
- 僅見「宋」而無南北宋線索者不強判

**腳本**：
- `scripts/fix_song_round5.py`

**已完成**：
| 類型 | 數量 | 說明 |
|---|---:|---|
| Author.dynasty 拆分 | 26 | `宋→北宋` 6、`宋→南宋` 20 |
| Work.dynasty 補齊 | 26 | `北宋` 6、`南宋` 20 |
| Work.period 補齊 | 26 | 北/南宋 → `song` |
| Entity.dynasty 拆分 | 11 | `北宋` 2、`南宋` 9 |
| index 同步 | 17 shards | `index/works` 10 shards，`index/entities` 7 shards |

**驗證**：
- 本輪觸及 Work：26，與 `index/works` 不一致：0
- 本輪觸及 Entity：11，與 `index/entities` 不一致：0
- `Work.dynasty=宋`：0

**剩餘未決**：
- `Author.dynasty=宋`：929
- `Entity.dynasty=宋`：491
- 詳見 `.claude/known-issues/宋代整理_round5_未決.json`
